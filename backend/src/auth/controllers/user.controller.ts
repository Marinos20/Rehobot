...
export class UserController {
    ...
    uploadImage(@UploadedFile() file: Express.Multer.File, @Request() req): Observable<{ modifiedFileName: string }> {
        if (!file) {
            ...
            switchMap((isFileLegit: boolean) => {
                if (isFileLegit) {
                    return this.userService.updateUserImageById(userId, fullImagePath).pipe(
                        switchMap(() => {
                            // Retourne l'URL de l'image modifiée
                            const avatarUrl = `/user/image/${file.filename}`;  // URL relative pour accéder à l'image
                            return of({ modifiedFileName: avatarUrl });
                        }),
                    );
                } else {
